...
public class ChineseCalendar
{
    ...
    /// 按公历计算的通用节假日
    /// </summary>
    private static HashSet<DateInfoStruct> SolarHolidayInfo { get; } = new()
    {
    ...
    /// 按农历计算的通用节假日
    /// </summary>
    private static HashSet<DateInfoStruct> LunarHolidayInfo { get; } = new()
    {
    ...
    /// <returns></returns>
    private bool BitTest32(int num, int bitPosition)
    {
        if (bitPosition is > 31 or < 0)
        {
            throw new ArgumentException("参数错误: 取值范围0-31", nameof(bitPosition));
        }

        var bit = 1 << bitPosition;
        return (num & bit) != 0;
    }
    ...
    public string ChineseCalendarHoliday
    {
        get
        {
            ...
            foreach (var date in from date in LunarHolidayInfo
                     let end = date.Recess > 0 ? date.Day + date.Recess - 1 : date.Day + date.Recess
                     where date.Month == ChineseMonth && ChineseDay >= date.Day && ChineseDay <= end
                     select date)
            {
        ...
        get
        {
            ...
            if (isHoliday)
            {
                return !isHoliday && !IsWeekend() ||
                    CustomWorkDays.Any(s => s.Date == Date);
            }

            {
                if ((from lh in LunarHolidayInfo
                        let end = lh.Recess > 0 ? lh.Day + lh.Recess - 1 : lh.Day + lh.Recess
                        where lh.Month == ChineseMonth && ChineseDay >= lh.Day && ChineseDay <= end && lh.Recess > 0
                        select lh).Any())
                {
                    isHoliday = true;
                }

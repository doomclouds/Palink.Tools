...
public class HttpCache
{
    ...
    /// </summary>
    public static HttpCache Current => (_instance ??= new HttpCache());
        ...
        TimeSpan expireIn, bool forceUpdate = false, bool throttled = true)
    {
        ...
        return client.SendCachedAsync(barrel, new HttpRequestMessage(HttpMethod.Get, url),
            expireIn, forceUpdate, throttled);
    }
}
...
{
    private static readonly SemaphoreSlim GetThrottle = new(4, 4);
        ...
        bool throttled = true)
    {
        ...
        try
        {
            if (!forceUpdate && !string.IsNullOrEmpty(eTag) &&
            ...
            if (r.StatusCode == HttpStatusCode.NotModified)
            {
                if (string.IsNullOrEmpty(contents))
                    throw new IndexOutOfRangeException(
                        $"Cached value missing for HTTP request: {url}");
        ...
        {
            // Cache it?
            var newEtag = r.Headers.ETag?.Tag;
        ...
        {
            throw new HttpCacheRequestException(r.StatusCode,
                "HTTP Cache Request Failed");
        }
    }
}

...
namespace Palink.Tools.NModbus.Extensions.Enron;
...
internal class RegisterFunctions
{
    public static byte[][] ReadRegisters(byte slaveAddress, ushort startAddress,
        ...
        Func<byte[], byte[]> endianConverter, bool wordSwap = false)
    {
        var registerMultiplier = GetRegisterMultiplier(wordSize);
        var registersToRead = (ushort)(numberOfPoints * registerMultiplier);
        var values =
            master.ReadHoldingRegisters(slaveAddress, startAddress, registersToRead);
        if (wordSwap) Array.Reverse(values);
        return ConvertRegistersToValues(values, registerMultiplier)
            .Select(endianConverter).ToArray();
    }

    public static void WriteRegistersFunc(byte slaveAddress, ushort startAddress,
        byte[][] data, IModbusMaster master, uint wordSize,
        Func<byte[], byte[]> endianConverter, bool wordSwap = false)
    {
        var wordByteArraySize = GetRegisterMultiplier(wordSize) * 2;
        if (data.Any(e => e.Length != wordByteArraySize))
        {
            throw new ArgumentException(
                "All data values must be of the correct word length.");
        }

        var dataCorrectEndian = data.Select(endianConverter).ToArray();
        var registerValues =
            ConvertValuesToRegisters(dataCorrectEndian);
        if (wordSwap) Array.Reverse(registerValues);
        master.WriteMultipleRegisters(slaveAddress, startAddress, registerValues);
    }


    public static char[] ByteValueArraysToChars(byte[][] data, bool frontPadding = true,
        bool singleCharPerRegister = true)
    {
    ...
    public static ushort[] ByteValueArraysToUShorts(byte[][] data,
        bool frontPadding = true)
    {
        return frontPadding
    ...
    public static byte[][] CharsToByteValueArrays(char[] data, uint wordSize,
        bool frontPadding = true, bool singleCharPerRegister = true)
    {
        var bytesPerWord = GetRegisterMultiplier(wordSize) * 2;
        if (!singleCharPerRegister)
        {
            ...
            for (var index = 0; index < byteArray.Length; index++)
            {
                byteArray[index] = index < data.Length
                    ...
                    : Convert.ToByte('\0'); //Unicode Null Character
            }
            ...
            for (var index = 0; index < byteValueArrays.Length; index++)
            {
                ...
                byteValueArrays[index] =
                    new ArraySegment<byte>(byteArray, offset, bytesPerWord).ToArray();
            }
    ...
    public static byte[][]
        ShortsToByteValueArrays(short[] data, uint wordSize, bool frontPadding = true) =>
        data.Select(e =>
            PadBytesToWordSize(wordSize, BitConverter.GetBytes(e),
                frontPadding)).ToArray();

    public static byte[][] UShortsToByteValueArrays(ushort[] data, uint wordSize,
        bool frontPadding = true) => data.Select(e =>
        PadBytesToWordSize(wordSize, BitConverter.GetBytes(e),
            frontPadding)).ToArray();

    public static byte[][]
        IntToByteValueArrays(int[] data, uint wordSize, bool frontPadding = true) => data
        .Select(e =>
            PadBytesToWordSize(wordSize, BitConverter.GetBytes(e),
                frontPadding)).ToArray();

    public static byte[][]
        UIntToByteValueArrays(uint[] data, uint wordSize, bool frontPadding = true) =>
        data.Select(e =>
            PadBytesToWordSize(wordSize, BitConverter.GetBytes(e),
                frontPadding)).ToArray();

    public static byte[][]
        FloatToByteValueArrays(float[] data, uint wordSize, bool frontPadding = true) =>
        data.Select(e =>
            PadBytesToWordSize(wordSize, BitConverter.GetBytes(e),
                frontPadding)).ToArray();


    private static byte[] PadBytesToWordSize(uint wordSize, byte[] source,
        bool frontPadding)
    {
        var targetLength = GetRegisterMultiplier(wordSize) * 2;
    ...
    private static byte[][] ConvertRegistersToValues(ushort[] registers,
        int registerMultiplier)
    {
        if ((registers.Length % registerMultiplier) != 0)
        {
            throw new InvalidDataException(
                "registers.Length is not a multiple of RegisterMultiplier");
        }
    ...
    private static int GetRegisterMultiplier(uint wordSize)
    {
        return wordSize switch
        {
            ...
            (32) => 2,
            (64) => 4,
            _ => throw new ArgumentException("Word size mus be 16/32/64")
        };
    }
}
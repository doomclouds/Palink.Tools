...
internal class RegisterCollection : Collection<ushort>, IModbusMessageDataCollection
{
    ...
    public RegisterCollection(byte[] bytes)
        : this((IList<ushort>)CoreTool.NetworkBytesToHostUInt16(bytes))
    {
    }
        ...
        get
        {
            ...
            foreach (var register in this)
            {
                var b = BitConverter.GetBytes(
                    (ushort)IPAddress.HostToNetworkOrder((short)register));
    ...
    public override string ToString()
    {
        return string.Concat("{",
            string.Join(", ", this.Select(v => v.ToString()).ToArray()), "}");
    }
}